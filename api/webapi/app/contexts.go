...
// NewDeleteContentContext parses the incoming request URL and body, performs validations and creates the
// context used by the content controller delete action.
func NewDeleteContentContext(ctx context.Context, r *http.Request, service *goa.Service) (*DeleteContentContext, error) {
	var err error
	resp := goa.ContextResponse(ctx)
	resp.Service = service
	req := goa.ContextRequest(ctx)
	req.Request = r
	rctx := DeleteContentContext{Context: ctx, ResponseData: resp, RequestData: req}
	paramID := req.Params["id"]
	if len(paramID) > 0 {
		rawID := paramID[0]
		if id, err2 := strconv.Atoi(rawID); err2 == nil {
...
// OK sends a HTTP response with status code 200.
func (ctx *DeleteContentContext) OK(resp []byte) error {
	if ctx.ResponseData.Header().Get("Content-Type") == "" {
		ctx.ResponseData.Header().Set("Content-Type", "text/plain")
	}
	ctx.ResponseData.WriteHeader(200)
...
// BadRequest sends a HTTP response with status code 400.
func (ctx *DeleteContentContext) BadRequest() error {
	ctx.ResponseData.WriteHeader(400)
	return nil
}

// NotFound sends a HTTP response with status code 404.
func (ctx *DeleteContentContext) NotFound() error {
	ctx.ResponseData.WriteHeader(404)
	return nil
}
...
// DeleteThreadsContext provides the threads delete action context.
type DeleteThreadsContext struct {
	context.Context
	*goa.ResponseData
	*goa.RequestData
	ID int
...
// NewDeleteThreadsContext parses the incoming request URL and body, performs validations and creates the
// context used by the threads controller delete action.
func NewDeleteThreadsContext(ctx context.Context, r *http.Request, service *goa.Service) (*DeleteThreadsContext, error) {
	var err error
	resp := goa.ContextResponse(ctx)
	resp.Service = service
	req := goa.ContextRequest(ctx)
	req.Request = r
	rctx := DeleteThreadsContext{Context: ctx, ResponseData: resp, RequestData: req}
	paramID := req.Params["id"]
	if len(paramID) > 0 {
		rawID := paramID[0]
		if id, err2 := strconv.Atoi(rawID); err2 == nil {
...
// OK sends a HTTP response with status code 200.
func (ctx *DeleteThreadsContext) OK(resp []byte) error {
	if ctx.ResponseData.Header().Get("Content-Type") == "" {
		ctx.ResponseData.Header().Set("Content-Type", "text/plain")
	}
	ctx.ResponseData.WriteHeader(200)
